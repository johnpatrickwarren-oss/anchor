...
| Coordinator (multi-cluster) | [`CLUSTER-HANDOFF-TEMPLATE.md`](./templates/CLUSTER-HANDOFF-TEMPLATE.md) + [`WAVE-GATE-TEMPLATE.md`](./templates/WAVE-GATE-TEMPLATE.md) | **Cluster-handoff** (one per directed dependency edge): source/target work units, dependency test that fired, interface contract description (schema/API/UI/file), verification status, target-cluster anti-assume list, pre-flags from wave gate, halt conditions, Coordinator verification log, amendment history. **Wave-gate** (one per wave): pre-advance checklist (completeness / quality / scope integrity / memorial), per-cluster findings with disposition (ADVANCE/RETRY/SCOPE-REDUCE-V1/ROUTE-TO-ARCHITECT), failure handling log, resequencing decisions, pre-flags to next wave, handoff status, coordinator memorial update, Wave N+1 dispatch authorization. |
