...
#
# CANONICAL COPY. The toolkit-root `finalize-round.sh` is a thin exec shim to
# this file; `new-project.sh` ships this copy (via `scripts/`) into projects,
# and `check-pipeline-sync.sh` syncs it.
#
...
# The `--` separator is load-bearing: without it, a configured source dir
# that does not exist in the working tree (e.g. prisma/ on the default
# scaffold) makes git exit 128 ("no such path"), which used to be
# misreported as "uncommitted changes". With `--`, absent pathspecs simply
# match nothing. git stderr is NOT suppressed, so a real git failure is
# distinguishable from a dirty tree.
if ! git diff --quiet -- "${SOURCE_DIRS[@]}"; then
...
if ! git diff --cached --quiet -- "${SOURCE_DIRS[@]}"; then
...
# Remove the pipeline lock before staging so SHA-A captures the deletion;
# the EXIT trap in run-pipeline.sh becomes a no-op on success.
...
# Stage FIRST, then decide from the index whether anything needs committing.
# A round's primary output is brand-new files (specs, reviews, summaries);
# any detection that filters untracked files (the old `grep -v "^??"`)
# silently excludes exactly those artifacts from the attestation.
ADD_PATHS=("coordination/")
...
if git diff --cached --quiet -- coordination/ "${CLAUDE_PATHS[@]}"; then
...
if ! git diff --quiet "$SHA_A" HEAD -- "${SOURCE_DIRS[@]}"; then
