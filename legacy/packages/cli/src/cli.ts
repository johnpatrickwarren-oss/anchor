...
import { defaultContext, cmdRoute, cmdRun, cmdMemorial, cmdWave, cmdCalibrate, cmdInit, cmdProject } from './commands.ts';
...
Usage:
  ...
  anchor project (--directive <file> | --task "<text>") [--repo <dir> [--base <ref>]] [--project-id <id>] [--concurrency <n>] [--memorial <path>] [--dry-run] [--mock]
                                                                         # decompose a whole project → run features in dependency-aware stages (auto role-scaling + auto parallelism). --dry-run shows the staged plan
...
export async function main(argv: string[]): Promise<number> {
  ...
  switch (cmd) {
    ...
    case 'project': return (await cmdProject(flags, ctx)).code;
