...
> It's its own repo now: **[johnpatrickwarren-oss/arch-gate](https://github.com/johnpatrickwarren-oss/arch-gate)** — install with `npm i -g github:johnpatrickwarren-oss/arch-gate`.
>
...
| [`@anchor/cli`](packages/cli) | `anchor init / project / run / wave / route / memorial / calibrate` — the operator surface. |
...
node packages/cli/src/cli.ts project --directive PROJECT.md --repo ./work --dry-run        # AUTO-decompose a project → staged plan (independent features parallel, dependents sequenced)
...
**Three levels of automatic scaling, one directive in:** the tier-router scales the **role set** per round (mechanical → just the implementer; complex → the full cold-eye cycle); `anchor project` decomposes a **whole project** into features and runs independent ones **in parallel stages**, sequencing dependents; and within a feature the Architect can declare file-disjoint **units** for concurrent sub-implementers. No hand-authored plan — give it the scope, it decides the roles *and* the parallelism. (Proof: [`case-studies/project-mode-strkit/`](case-studies/project-mode-strkit/) — a 5-feature project auto-decomposed into 3 dependency-aware stages, built live, 57/57 tests green.)
